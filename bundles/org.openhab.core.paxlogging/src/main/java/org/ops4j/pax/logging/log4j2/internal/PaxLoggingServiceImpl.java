...
/**
 * NOTE: this code includes two copyright headers; one for the original in {@code org.ops4j.pax.logging.log4j2.internal}
 * and one for the fragment modifications applied here in openHAB.
 * <p>
 * This is an override fragment for {@code org.ops4j.pax.logging.log4j2.internal.PaxLoggingServiceImpl} for openHAB
 * purposes. When the initial valid configuration is provided by Karaf, instead of setting it immediately, it store the
 * configuration the {@code deferredConfiguration} field. And then after the OSGi framework has fully started and the
 * console is stable, openHAB core actually set that configuration by calling {@code setDeferredConfiguration()} method
 * via reflection.
 * <p>
 * Eliminates console "Terminal has been closed" bug that otherwise occurs on initial start up or after cache clean.
 * 
 * @author Niclas Hedhman - Initial contribution
 * @author Andrew Fiddian-Green - openHAB specific fragment modifications
 */
public class PaxLoggingServiceImpl implements PaxLoggingService, ServiceFactory<Object> {
    ...
    public PaxLoggingServiceImpl(BundleContext bundleContext, LogReaderServiceImpl logReader,
        ...
        String errorsAsExceptionValue = OsgiUtil.systemOrContextProperty(bundleContext,
                PaxLoggingConstants.LOGGING_CFG_LOG4J2_ERRORS_AS_EXCEPTION);
    ...
    private <L extends org.osgi.service.log.Logger> L getLogger(Bundle bundle, String name, Class<L> loggerType,
            String fqcn) {
     ...
     * @param category
     * @param fqcn
     * @param printfFormatting whether to use Slf4J ({@code "{}"} - {@code false}) or printf formatting ({@code "%s"} -
     *            {@code true}).
     * @return
     */
        ...
        if (async) {
            try {
                ...
                StatusLogger.getLogger().warn(
                        "Asynchronous loggers defined, but the disruptor library is not available.  Reverting to synchronous loggers.",
                        e);
                async = false;
            }
        }
    ...
    void handleEvents(String name, Bundle bundle, ServiceReference<?> sr, LogLevel level, String message,
            Throwable exception) {
        LogEntry entry = m_logReader != null || m_eventAdmin != null
                ? new LogEntryImpl(name, bundle, sr, level, message, exception)
                : null;
        if (m_logReader != null) {
    ...
    private void configureLog4J2(boolean async, String configFileName, Dictionary<String, ?> configuration) {
        ...
        if (file != null && !file.isFile()) {
            if (configFileName.contains(",")) {
                ...
                StatusLogger.getLogger().warn(
                        "Configuration file '" + file + "' is not available. Default configuration will be used.");
                file = null;
                configFileName = null;
            }
        }
        ...
        try {
            ...
            if (m_log4jContext == null || async != m_async) {
                m_log4jContext = async ? new AsyncLoggerContext(LOGGER_CONTEXT_NAME)
                        : new LoggerContext(LOGGER_CONTEXT_NAME);
                m_async = async;
            }
            ...
            try {
                ...
                if (props != null) {
                    ...
                    // or as single, "org.ops4j.pax.logging.log4j2.config.file" property in
    ...
    private void setLevelToJavaLogging() {
        for (Enumeration<String> enum_ = java.util.logging.LogManager.getLogManager().getLoggerNames(); enum_
                .hasMoreElements();) {
        ...
        for (Logger logger : m_log4jContext.getLoggers()) {
            if (logger != null) {
                ...
                if (logger.getName().equals(LogManager.ROOT_LOGGER_NAME)) {
                    ...
                    // "global" comes from java.util.logging.Logger.GLOBAL_LOGGER_NAME, but that constant wasn't added
                    // until Java 1.6
     ...
     * ========= OPENHAB CODE =========
     * <p>
     * Saves the provided configuration as deferred configuration, if the OSGi framework is not yet fully started and
     * the console is not yet stable.
     *
     * @param configuration the configuration to be saved as the deferred configuration
     ...
     * ========= OPENHAB CODE =========
     * <p>
     * This method is called by openHAB core (via reflection) after the OSGi framework has fully started and the
     * console is stable. It sets the deferred Pax Logging configuration (if any) that was provided by Karaf
